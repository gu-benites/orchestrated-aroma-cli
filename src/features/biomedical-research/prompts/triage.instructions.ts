...
export const triageAgentConfig: Partial<AgentConfiguration> = {
  ...
  modelSettings: {
    ...
    toolChoice: 'none', // This agent must not use tools, only handoff.
  },
...
1.  **If the query contains a PubMed ID (PMID)**, which is typically an 8-digit number (e.g., "38155861", "details on 12345678"), immediately hand off to the **PMID Details Specialist**.
2.  **For ALL other queries** related to biomedical research (e.g., "what essential oils help with anxiety?", "studies on lavender and linalool"), immediately hand off to the **Biomedical Search Specialist**.

## Critical Instructions:
- **DO NOT** attempt to answer the user's query yourself.
- **DO NOT** ask clarifying questions.
- Your one and only action is to perform a handoff.
